...
const ListingPagination = ({ totalPages }) => {
  ...
  return (
    <>
      <div className='pagination-area active-pagination text-center'>
        {[...Array(totalPages).keys()].map((x) => (
          <span key={x + 1}>
            <Link href={`/dashboard/my-listing/active/?page=${x + 1}`}>
              <a
                className={
        ...
        <Link href={`/dashboard/my-listing/active/?page=${page + 1}`}>
          <a
            className={
              page === totalPages
